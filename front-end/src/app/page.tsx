...
function EmbeddedChatbot() {
  ...
  const openCalendlyWithUserInfo = async (name: string, email: string) => {
    try {
      ...
      const eventTypesResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/calendly-event`, {
        method: "GET",
        headers: {
      ...
      const schedulingResponse = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/calendly-create-link`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
